...
// 기능 2. sexBtn click에 따라 userSex저장 및 profile img 변경 + sheet11 payNotice select
...
const manPayNotice = document.querySelector("#manPay");
const womanPayNotice = document.querySelector("#womanPay");
...
function onManBtnClick(event){
  ...
  manPayNotice.classList.add(SELECTED_CLASS);
  womanPayNotice.classList.remove(SELECTED_CLASS);
...
function onWomanBtnClick(event){
  ...
  manPayNotice.classList.remove(SELECTED_CLASS);
  womanPayNotice.classList.add(SELECTED_CLASS);
